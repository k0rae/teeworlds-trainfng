...
#define GAME_TYPE_NAME "trainfng"
#define TEST_TYPE_NAME "test-trainfng"
...
void CGameControllerDDRace::OnPlayerConnect(CPlayer *pPlayer)
{
	...
	{
		char aBuf[512];
		str_format(aBuf, sizeof(aBuf), "'%s' entered and joined the %s with %s client", Server()->ClientName(ClientID), GetTeamName(pPlayer->GetTeam()), Server()->IsSixup(ClientID) ? "0.7" : "0.6");
		GameServer()->SendChat(-1, CGameContext::CHAT_ALL, aBuf, -1, CGameContext::CHAT_SIX | CGameContext::CHAT_SIXUP);

		GameServer()->SendChatTarget(ClientID, "TrainFNG Mod. Version: " GAME_VERSION);
		GameServer()->SendChatTarget(ClientID, "Say /info for more info");
	}
}
